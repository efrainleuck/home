...
options = [
  ...
  Option [] ["quiet"] (NoArg (\opts -> opts {optQuiet = True})) "Only print error messages on tests, or minimal output when solving.",
  ...
  then do
       ...
       then helpIO
            ...
            then do 
                    ...
                    runTests  trainingImages allValidationImages allValidationLabels (optQuiet opts) (optForceTests opts)
...
evalResults opts toDigit protoResults =  do
    ...
    let verb = verbocity opts > 0 && not (optQuiet opts)
